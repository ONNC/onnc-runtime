...
void ONNC_RUNTIME_logsoftmax_float(
  ...
  ,int32_t axis
    ...
    for(int32_t i = 0   ; i < axis            ; ++i) size_N *= input_input_dims[i];
    for(int32_t i = axis; i < input_input_ndim; ++i) size_D *= input_input_dims[i];
    ...
    for(int32_t iN = 0; iN < size_N; ++iN){
        ...
        for(int32_t i = 0; i < size_D; ++i) maxData = fmaxf(maxData, pInput[i]);
        ...
        for(int32_t i = 0; i < size_D; ++i) pOutput[i] = logf(pOutput[i] / sumData);
    }
}
