...
static inline void pad_along_axis(
    ...
    int32_t edge_axis_start_index = object_area[axis];
    int32_t edge_axis_end_index = object_area[axis + output_ndim];
    switch(mode){
        case EDGE_MODE:{
            do{
...
static inline void update_object_area(int32_t * restrict object_area, int32_t output_ndim, const int32_t * restrict output_dims, int32_t axis){
    object_area[axis] = 0;
    object_area[axis+output_ndim] = output_dims[axis];
}
...
static inline void ONNC_RUNTIME_pad_float(
