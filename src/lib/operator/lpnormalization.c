...
void ONNC_RUNTIME_lpnormalization_float(
  ...
  int32_t in_coor[input_elem][ndim];
  int32_t out_coor[output_elem][out_ndim];
  ...
  for(int32_t j = 0; j < input_elem; j++){
    ...
    summation[outIndexCounter] += fabs(pow(input_input[j], p));
  }

  //Formula
  for(int32_t i = 0; i < output_elem; i++){
    output_output[i] = pow(summation[i], (1.0f/(float)p)); 
  }
}
