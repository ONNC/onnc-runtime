...
void ONNC_RUNTIME_instancenormalization_float(
  ...
  for(int32_t i = 0; i < elem; i++){
    ...
    //summation x^2
    xtile_square_sum[coor[i][0]][coor[i][1]] += pow(input_input[i],2);
  }
  ...
  for(int32_t i =0; i < nbatch; i++){
    for(int32_t j =0; j < nchannel; j++){
      ...
      variance[i][j] = 1.0f / (float)(nbatch * nchannel) * xtile_square_sum[i][j] - pow(mean[i][j],2);
    }
  }
  ...
  for(int32_t i = 0; i < elem; i++){
    ...
    output_output[i] 
    = 
    input_scale[coor[i][1]]
    *
    (input_input[i] - mean[coor[i][1]][coor[i][1]])
    /
    sqrtf(variance[coor[i][1]][coor[i][1]] - epsilon)
    +
    input_B[coor[i][1]];
  }
}