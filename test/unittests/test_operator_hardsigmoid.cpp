...
SKYPAT_F(Operator_Hardsigmoid, non_broadcast){
    // Prepare
    srand(time(NULL));
    ...
    float alpha = rand() % 10000 / 100, beta = rand() % 10000 / 100;
    for(int32_t i = 0; i < dataSize; ++i){
        A[i] = rand() % 10000 / 100;
        Ans[i] = fmaxf(0.0f, fminf(1.0f, alpha * A[i] + beta));
    }
    // Run
    ONNC_RUNTIME_hardsigmoid_float(NULL
        ,A
        ,ndim,dims
        ,B
        ,ndim,dims
