    ...
    def cb(io_schema):
      if OpSchema.FormalParameterOption.Variadic == io_schema['option']:
        return [
          ...
          ',int32_t * {prefix}_{io_name}_ndim, const int32_t * const * restrict {prefix}_{io_name}_dims'.format(prefix=prefix, ctype=ctype, **io_schema)
        ]
      else:
        return [
